...
class CityStatusChange extends Action
{
    ...
    public function handle(ActionFields $fields, Collection $models)
    {
        ...
        if (count($requestIds) > 0) {
            DB::table('cities')->whereIn('id', $requestIds)->update([
                'status' => $status,
                'updated_by' => Auth::id()
            ]);
            if ($status == 'inactive') {
                DB::table('localities')->whereIn('city_id', $requestIds)->update(['status' => 'inactive']);
                DB::table('posts')->whereIn('city_id', $requestIds)->update(['status' => 'inactive']);
            }
