...
class LocalityStatusChange extends Action
{
    ...
    public function handle(ActionFields $fields, Collection $models)
    {
        ...
        if (count($requestIds) > 0) {
            DB::table('localities')->whereIn('id', $requestIds)->update([
                'status' => $status,
                'updated_by' => Auth::id()
            ]);
            if ($status == 'inactive') {
                DB::table('posts')->whereIn('locality_id', $requestIds)->update(['status' => 'inactive']);
            }
